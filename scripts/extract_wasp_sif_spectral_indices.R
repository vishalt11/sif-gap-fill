...
output_rds <- file.path(
  "data",
  paste0("ba_sif_", target_tile, "_wasp_spectral_indices.rds")
)
output_csv <- file.path(
  "data",
  paste0("ba_sif_", target_tile, "_wasp_spectral_indices.csv")
)
...
extract_index_summary <- function(index_id, product_dir, product_id, flg_r1, flg_r2, sif_polygons_utm) {
...
process_product <- function(product_id, product_dir, year_month) {
  ...
  rm(flg_r1, flg_r2, sif_polygons, sif_polygons_utm, index_summaries)
  gc(verbose = FALSE)

  product_result
}

ba_sif_wasp_spectral_indices <- pmap(
...
if (nrow(ba_sif_wasp_spectral_indices) == 0) {
  stop("No SIF rows matched the local WASP products under ", wasp_root, call. = FALSE)
}

saveRDS(ba_sif_wasp_spectral_indices, output_rds)
