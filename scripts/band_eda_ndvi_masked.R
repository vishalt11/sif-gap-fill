...
levels(crop_raster_2020) <- data.frame(value = crop_classes$code, crop = crop_classes$label)

target_crop_crs <- target_ns_32upc %>% st_transform(crs(crop_raster_2020)) %>% vect()

crop_raster_target <- crop(crop_raster_2020, target_crop_crs) %>% mask(target_crop_crs)
...
# sif_labels <- sprintf(
#   "<strong>SIF ID:</strong> %d<br/> <strong>SIF:</strong> %.4f<br/><strong>Date:</strong> %s<br/><strong>MGRS:</strong> %s<br/><strong>WW share:</strong> %.1f%%",
#   sif_polygons_2020$sif_id,
