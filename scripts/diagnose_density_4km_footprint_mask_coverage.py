"""Diagnose footprint coverage inside density-based 4 km SIF windows.
...
projected to the 4 km window CRS, and rasterized on the same 20 m grid that
would later be used for model preparation.

A footprint passes when at least 60% of its rasterized area is inside its
assigned window. A window is retained after removing failed footprints when at
least four passing footprints remain. The script reports both this primary
...
MANIFEST_PATH = (
    AGGREGATION_DIR / "density_cluster_4000m_aggregate_manifest.csv"
)
ASSIGNMENTS_PATH = (
    AGGREGATION_DIR / "density_cluster_4000m_sif_assignments.csv"
)
...
def rasterized_inside_fraction(
    ...
    # Rasterization is automatically clipped to the 4 km output extent.
    high_mask = rasterize(
...
def diagnose_coverage() -> None:
    ...
    print(
        f"Diagnosing {total_windows:,} density-based 4 km windows and "
    ...
    for window_index, manifest_row in manifest.iterrows():
        ...
        for _, footprint in footprints.iterrows():
            ...
            diagnostic_row = {
                ...
                "passes_60pct_mask_inside": passes,
            }
        ...
        passing = window_coverage["passes_60pct_mask_inside"]
        ...
        output_row.update(
            {
                ...
                "all_original_footprints_pass_60pct": strict_all_pass,
                "window_retained_after_60pct_filter": retained_window,
                "window_lost_after_60pct_filter": not retained_window,
    ...
    retained_windows = int(
        window_diagnostics["window_retained_after_60pct_filter"].sum()
    )
    lost_windows = original_windows - retained_windows
    strict_windows = int(
        window_diagnostics["all_original_footprints_pass_60pct"].sum()
    )
    original_footprints = len(footprint_diagnostics)
    retained_footprints = int(
        footprint_diagnostics["passes_60pct_mask_inside"].sum()
    )
    ...
    retained_count_distribution = (
        window_diagnostics.groupby(
            [
                ...
                "window_retained_after_60pct_filter",
            ],
            dropna=False,
        )
        .size()
    ...
    print(
        "Windows retained after removing <60% footprints and requiring "
    ...
    print(
        "Strict windows where every assigned footprint passes 60%: "
