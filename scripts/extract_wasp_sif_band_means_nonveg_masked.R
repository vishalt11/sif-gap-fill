...
output_rds <- file.path(
  "data/band_means_nonveg_masked",
  paste0(state_prefix, "_sif_", target_tile, "_wasp_band_means_nonveg_masked.rds")
)
output_csv <- file.path(
  "data/band_means_nonveg_masked",
  paste0(state_prefix, "_sif_", target_tile, "_wasp_band_means_nonveg_masked.csv")
)
...
extract_band_summary <- function(
  ...
  contamination_mask <- if (band_group == "R1") {
    contamination_masks$mask_10m
  } else {
    contamination_masks$mask_20m
  }

  band <- load_scaled_land_band(product_dir, product_id, band_id, flg, sif_vect)
  band <- apply_contamination_mask(band, contamination_mask)
...
process_product <- function(product_id, product_dir, product_year, year_month) {
  ...
  rm(
    ...
    contamination_masks, band_summaries
  )
  gc(verbose = FALSE)
...
sif_wasp_band_means_nonveg_masked <- pmap(
...
if (nrow(sif_wasp_band_means_nonveg_masked) == 0) {
  stop("No SIF rows matched the local WASP products under ", wasp_root, call. = FALSE)
}

saveRDS(sif_wasp_band_means_nonveg_masked, output_rds)

sif_wasp_band_means_nonveg_masked %>%
  st_drop_geometry() %>%
