"""Build density-aggregated 4 km CNN chips using 20 m SNAP FAPAR.

The input tables are the sampled manifest and matching footprint assignments
in data/density_aggregation/sen2_spataggr_fapar. Each output sample contains:
...
SNAP FAPAR is read from the matching window GeoTIFF in physical units 0-1,
without rescaling or reprojection. Flags 0-3 are accepted, including warnings.
Missing FAPAR is filled using iterative 3x3 neighbour means before calculating
APAR. Valid observations, including zero, are preserved. No FAPAR coverage
threshold is imposed and no flags or validity masks are written to the NPZs.
An entirely missing FAPAR window raises an error because there is no local
value from which to fill. FAPAR and spectral composites may select different
acquisitions; per-pixel date alignment cannot be checked from these outputs.
...
OUTPUT_DIR = (
    ...
    / "data"
    / "cnn_sentinel2_chips"
    / "spatial_aggregate_density_4km_20m_snap_fapar"
)
...
FAPAR_DIR = PROJECT_ROOT / "data" / "sentinel2_fapar_composite_snap"
ACCEPTED_FAPAR_FLAGS = (0, 1, 2, 3)
...
# Reuse the established PAR, crop and footprint-mask routines at the
...
def preflight_predictor_files(manifest: pd.DataFrame) -> None:
    ...
    for aggregation_id in manifest["aggregation_id"]:
        for path in snap_fapar_paths(str(aggregation_id)):
            if not path.is_file():
                raise FileNotFoundError(f"Missing SNAP FAPAR input: {path}")
    ...
    print(
        ...
        f"{len(manifest):,} SNAP FAPAR/flag pairs and "
...
# ---------------------------------------------------------------------------
# SNAP FAPAR on the existing window grid


def snap_fapar_paths(aggregation_id: str) -> tuple[Path, Path]:
    return (
        FAPAR_DIR / f"{aggregation_id}_fapar.tif",
        FAPAR_DIR / f"{aggregation_id}_snap_flag.tif",
    )


def fill_missing_fapar(values: np.ndarray, aggregation_id: str) -> np.ndarray:
    """Fill only missing pixels, expanding from available 8-neighbour means."""
    filled = values.copy()
    valid = np.isfinite(filled)
    if not valid.any():
        raise ValueError(f"{aggregation_id}: no valid SNAP FAPAR pixels to fill from")
    ...
    while not valid.all():
        ...
        sums = convolve(
            np.where(valid, filled, 0.0), kernel, mode="constant", cval=0.0
        )
        fillable = ~valid & (counts > 0)
        if not fillable.any():
            raise RuntimeError(f"{aggregation_id}: SNAP FAPAR neighbour filling stalled")
...
def read_snap_fapar(
    aggregation_id: str, dst_transform, dst_crs
) -> tuple[np.ndarray, Path]:
    fapar_path, flag_path = snap_fapar_paths(aggregation_id)
    arrays = []
    for path in (fapar_path, flag_path):
        with rasterio.open(path) as src:
            if (src.count, src.height, src.width) != (1, CHIP_SIZE, CHIP_SIZE):
                raise ValueError(f"{aggregation_id}: expected a single-band 200 x 200 raster: {path}")
            if src.crs != dst_crs or not np.allclose(
                tuple(src.transform), tuple(dst_transform), rtol=0.0, atol=1e-6
            ):
                raise ValueError(f"{aggregation_id}: SNAP raster grid differs from Sentinel chip: {path}")
            # Cast before filling so UInt8 flag NoData can also become NaN.
            arrays.append(src.read(1, masked=True).astype(np.float32).filled(np.nan))

    fapar, flags = arrays
    valid = (
        np.isfinite(fapar) & (fapar >= 0.0) & (fapar <= 1.0)
        & np.isin(flags, ACCEPTED_FAPAR_FLAGS)
    )
    fapar[~valid] = np.nan
    # Physical FAPAR is already 0-1; do not divide by 10,000.
    return fill_missing_fapar(fapar, aggregation_id), fapar_path
...
def build_predictor_chip(
    manifest_row: pd.Series,
) -> tuple[np.ndarray, object, object, Path, Path]:
    aggregation_id = str(manifest_row["aggregation_id"])
    ...
    fapar, fapar_path = read_snap_fapar(aggregation_id, dst_transform, dst_crs)
    ...
    apar = fapar * par  # Use the filled SNAP FAPAR, with PAR on the SIF date.
    ...
    # FAPAR is filled; any missing values in other channels are preserved.
    x = np.stack(channels, axis=0).astype(np.float16)
    return x, dst_transform, dst_crs, raster_path, fapar_path
...
def build_sample(
    ...
    x, dst_transform, dst_crs, raster_path, fapar_path = build_predictor_chip(
        manifest_row
    )
    ...
    metadata = {
        ...
        "fapar_raster_path": str(fapar_path),
...
def prepare_chips() -> None:
    ...
    config = {
        ...
        "fapar_source": "Sentinel-2 SNAP biophysical FAPAR",
        "fapar_directory": str(FAPAR_DIR),
        "fapar_units": "physical fraction 0-1; no scaling applied",
        "fapar_accepted_flags": list(ACCEPTED_FAPAR_FLAGS),
        "fapar_temporal_matching": "precomputed per-window composite within +/-8 UTC days of SIF Delta_Date",
        "fapar_spectral_date_alignment": "not enforced; independently selected pixel acquisitions may differ",
        "fapar_resampling": "none; require the exact Sentinel chip grid",
        "fapar_gap_fill": "iterative 3x3 valid-neighbour mean; preserve observed values including zero",
        "fapar_all_missing_policy": "raise an error; no local values available for filling",
        "apar_formula": "filled SNAP FAPAR * daily PAR",
        ...
        "predictor_nan_policy": (
            "fill SNAP FAPAR locally; preserve other missing predictors; "
            "after splitting, compute nan-aware normalization from "
        ...
        "ancillary_spatial_check": (
            "SNAP FAPAR must have at least one valid pixel before local filling; "
            "PAR and crop channels must each contain at least one finite pixel; "
            "no FAPAR coverage threshold or separate coverage scan"
        ),
