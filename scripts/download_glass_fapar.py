...
OUTPUT_DIR = Path("data/glass_fapar_modis_250m")
CHECKLIST_PATH = OUTPUT_DIR / "glass_fapar_download_checklist.csv"
...
USER_AGENT = "Mozilla/5.0 GLASS-FAPAR-downloader"
...
def target_days() -> list[Target]:
    ...
    for year in YEARS:
        for doy in range(START_DOY, END_DOY + 1, DOY_STEP):
            for tile in TILES:
                targets.append(Target(year=year, doy=doy, tile=tile.lower()))
