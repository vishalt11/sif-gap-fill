...
def load_checklist(target_months: list[str]) -> pd.DataFrame:
    ...
    # Empty CSV columns are often inferred as float64 by pandas. Explicitly
    # keep fields that later receive strings as object columns so status,
    # paths, errors and ISO timestamps can be assigned safely.
    text_columns = [
        "item_id",
        "identifier",
        "archive_title",
    ...
    for column in text_columns:
        checklist[column] = checklist[column].astype("object")
...
def main() -> None:
    ...
    checklist = load_checklist(requested_months)
    save_checklist(checklist)
    ...
    print(f"Tile-month products requested: {len(requested_months)}")
