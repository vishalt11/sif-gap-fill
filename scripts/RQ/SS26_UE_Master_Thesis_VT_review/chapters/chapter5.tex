...
where $f_{\theta}$ represents a fitted tree-based, multilayer-perceptron, or CNN model with parameters $\theta$, $u$ denotes the spatial support or OCO-2 footprint, $t$ the observation time, $\mathbf{X}_{u,t}$ the predictor variables, and $S_{u,t}$ the OCO-2-derived target (this input target representations can vary from a single data point to spatial aggregates combined multiple targets into one value, this will be discussed in depth in ~\ref{sec:input_data}). For area-to-point MLP, $\mathbf{X}_{u,t}$ was a footprint-level matrix containing the predictor values for every 20 m pixel, For polygon-mean models, $\mathbf{X}_{u,t}$ was a tabular summary of footprint-level statistics, For CNN models, $\mathbf{X}_{u,t}$ was a multi-channel raster chip.
...
\begin{figure}[p]
    ...
    \caption{Sample 20m enhanced SIF map for selected models. The CNN models are able to capture more complex shapes compared to the tree based models which delineate the field structure exactly.}
